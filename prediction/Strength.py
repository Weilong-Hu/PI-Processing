...
def calc_descriptors_from_smiles(smiles: str):
    ...
    # ===== 按指定顺序添加 =====
    # 1. RDKit 描述符
    desc_values.append(Descriptors.MinAbsEStateIndex(mol))   # 1
    desc_values.append(Descriptors.SlogP_VSA11(mol))         # 2
    desc_values.append(Descriptors.SlogP_VSA2(mol))          # 3
    desc_values.append(Descriptors.fr_NH1(mol))              # 4
    desc_values.append(Descriptors.fr_ether(mol))            # 5

    # 2. Mordred 描述符
    mordred_names = [
        "SpDiam_A", "ATSC2dv", "AATSC0Z", "AATSC1Z", "SaaaC",
        "SssNH", "SssO", "AETA_beta", "AETA_beta_ns", "AETA_beta_ns_d",
        "fMF", "SIC5", "AMID_N", "nFaHRing", "RotRatio", "JGI9"
    ]
...
class StrengthGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Strength Predictor")
        self.root.geometry("400x220")
        self.smiles = ""
        self.tg = None
        self.n_trials = 0
        self.model = joblib.load("./Strength_model.pkl")  # 强度预测模型
    ...
    def run_optimization(self):
        ...
        def objective(trial):
            x22 = trial.suggest_int("x22", 3000, 30000)
            x23 = trial.suggest_int("x23", 30, 2500)
            x24 = trial.suggest_int("x24", 25, 500)
            x25 = trial.suggest_int("x25", 5, 1500)
            x26 = trial.suggest_int("x26", 1, 10)
            ...
            variable = np.array([x22, x23, x24, x25, x26])
    ...
    def show_result_window(self, best_y, best_params):
        for widget in self.root.winfo_children():
            widget.destroy()
        tk.Label(self.root, text=f"预测拉伸强度(MPa)：{best_y:.6f}", font=("Microsoft YaHei", 12)).pack(pady=10)
